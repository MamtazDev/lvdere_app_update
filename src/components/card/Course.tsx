...
export default function Course(props: {
  ...
  const [putRating, { isLoading: giveRating }] = usePutRatingMutation();
  ...
  const handleUnenrolling = async () => {
    if (
      window.confirm(
        `Are you really want to cancle the enrollment of ${title}?`
      )
    ) {
      const response: any = await unenrollCourse(slug);
